...
@triton.jit
def _attn_fwd_inner_int8(acc, l_i, m_i, q, q_scale, kv_len,
...
@triton.jit
def _attn_fwd_inner_e4m3(acc, l_i, m_i, q, q_scale, kv_len,
                         K_ptrs, K_scale_ptr, V_ptrs, V_scale_ptr, stride_kn, stride_vn, 
                         start_m,  
                         BLOCK_M: tl.constexpr, HEAD_DIM: tl.constexpr, BLOCK_N: tl.constexpr,  
                         STAGE: tl.constexpr, offs_m: tl.constexpr, offs_n: tl.constexpr):
...
@triton.jit
def _attn_fwd_inner_e5m2(acc, l_i, m_i, q, q_scale, kv_len,
                         K_ptrs, K_scale_ptr, V_ptrs, V_scale_ptr, stride_kn, stride_vn, 
                         start_m,  
                         BLOCK_M: tl.constexpr, HEAD_DIM: tl.constexpr, BLOCK_N: tl.constexpr,  
                         STAGE: tl.constexpr, offs_m: tl.constexpr, offs_n: tl.constexpr):
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        p_e5m2 = (p / p_scale).to(tl.float8e5)

        v = tl.load(V_ptrs, mask = offs_n[:, None] < (kv_len - start_n))
        v_scale = tl.load(V_scale_ptr)

        accumulator = tl.zeros([BLOCK_M, HEAD_DIM], dtype=tl.float32)
        middle = tl.dot(p_e5m2, v, accumulator).to(tl.float32) * v_scale * p_scale
        acc = acc + middle
...
def _attn_fwd(Q, K, V, Q_scale, K_scale, V_scale, Out, Lse, 
    ...
    if V.dtype == torch.int8:
        acc, l_i, m_i = _attn_fwd_inner_int8(acc, l_i, m_i, q, q_scale, kv_len, K_ptrs, K_scale_ptr, V_ptrs, V_scale_ptr, stride_kn, stride_vn,
                                             start_m, BLOCK_M, HEAD_DIM, BLOCK_N, 4 - STAGE, offs_m, offs_n)
    elif V.dtype == torch.float8e4nv:
        acc, l_i, m_i = _attn_fwd_inner_e4m3(acc, l_i, m_i, q, q_scale, kv_len, K_ptrs, K_scale_ptr, V_ptrs, V_scale_ptr, stride_kn, stride_vn,
                                             start_m, BLOCK_M, HEAD_DIM, BLOCK_N, 4 - STAGE, offs_m, offs_n)
    elif V.dtype == torch.float8e5:
        acc, l_i, m_i = _attn_fwd_inner_e5m2(acc, l_i, m_i, q, q_scale, kv_len, K_ptrs, K_scale_ptr, V_ptrs, V_scale_ptr, stride_kn, stride_vn,
                                             start_m, BLOCK_M, HEAD_DIM, BLOCK_N, 4 - STAGE, offs_m, offs_n)
    else:
...
def forward(q, k, v, 
    ...
    _attn_fwd[grid](
        q, k, v, q_scale, k_scale, v_scale, o, lse,
