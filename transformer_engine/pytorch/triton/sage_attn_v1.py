...
def _attn_bwd(Q, K, V,
    ...
    q_scale_offset = off_b * HEAD_NUM * tl.cdiv(SEQ_LEN, BLOCK_Q2) + off_h * tl.cdiv(SEQ_LEN, BLOCK_Q2)
    Q_scale_ptr = Q_scale + q_scale_offset + start_m  
    q_scale = tl.load(Q_scale_ptr)

    kv_scale_offset = off_b * (HEAD_NUM // GROUPS) * tl.cdiv(SEQ_LEN, BLOCK_KV1) + (off_h // GROUPS) * tl.cdiv(SEQ_LEN, BLOCK_KV1)
    ...
    MASK_BLOCK_KV_Q: tl.constexpr = BLOCK_KV2 // BLK_SLICE_FACTOR
    ...
    num_steps = BLOCK_Q2 // MASK_BLOCK_KV_Q
    dq = _attn_bwd_dq(dq, q, K, V, 
                      do, m, D, 
                      stride_tok, stride_d, 
                      HEAD_NUM, SEQ_LEN, 
                      BLOCK_Q2, MASK_BLOCK_KV_Q, HEAD_DIM, 
                      start_m, end_n - num_steps * MASK_BLOCK_KV_Q, num_steps, 
                      MASK=True 
                      )
    end_n -= num_steps * MASK_BLOCK_KV_Q
    # stage 2
...
class _attention(torch.autograd.Function):
    ...
    def backward(ctx, do):
        ...
        grid = (SEQ_L // BLOCK_KV1, 1, BATCH * N_HEAD)
        _attn_bwd[grid](
            q, k, v, 
            q_scale, k_scale, v_scale,
            do, dq, dk, dv, 
